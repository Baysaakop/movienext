...
const MovieCrew = ({ id }) => {
    ...
    useEffect(() => {
        axios({
            method: 'GET',
            url: `${api.moviecrew}?movie=${id}&first=8`
        })
        .then(res => {                   
        ...
        <div>            
            <div className={styles.container}>
                <div className={styles.header}>                                        
                    <Typography.Title level={5} style={{ margin: 0 }}>Уран бүтээлчид</Typography.Title>
                    <div>
                                    ...
                                    <List.Item key={item.id} className={styles.sliderItem}>                        
                                        <Link href={`/artists/${item.artist.id}`}>
                                            ...
                                            >
                                                ...
                                                <div className={styles.artistRole}>{item.roles[0].name}</div>          
                                            </Card>
                                        </Link>
                                    </List.Item>
                                )}
